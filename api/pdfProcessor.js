...
export class PDFProcessor {
  ...
  async extractPages(pdfBuffer, pageNumbers, options = {}) {
    const { filename = 'extracted.pdf' } = options;
  ...
  async splitPages(pdfBuffer, segments, options = {}) {
    const { baseFilename = 'page' } = options;
    ...
    for (const segment of segments) {
      ...
      const result = await this.extractPages(pdfBuffer, segment, { filename });
      results.push({
        ...result,
  ...
  async createZip(files) {
    // For serverless environment, we'll return individual files
    // Client will handle creating ZIP on the frontend
    return files;
  }
